...
Please provide your evaluation in the following JSON format:
{
  ...
  "feedbackExplanation": "A detailed explanation of errors (grammar, vocabulary, style, relevance). Give specific examples from the student's text. Explain corrections. Offer suggestions for improvement. Be encouraging. Tailor explanations to the student's language level. If the answer is good, highlight its strengths.",
  "overallAssessment": "A concise (1-2 sentences) overall assessment of the answer's quality."
}
  ...
  async (input) => {
    ...
    if (!output) {
      ...
      return {
        ...
        feedbackExplanation: "Die automatische Bewertung ist fehlgeschlagen. Bitte versuche es später erneut oder bitte einen Lehrer um Hilfe.",
        overallAssessment: "Bewertung nicht möglich."
      };
    }
    ...
    try {
      ...
      // Fallback if parsing fails
       return {
        correctedAnswer: input.userAnswer,
        feedbackExplanation: "Fehler bei der Verarbeitung der KI-Antwort. Bitte versuche es später erneut.",
        overallAssessment: "Bewertung fehlgeschlagen."
      };
    }
  }
);
